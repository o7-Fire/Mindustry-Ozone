...
public class JoinDialog extends BaseDialog {
	...
	public JoinDialog() {
		...
		TextField field = add.cont.field(Core.settings.getString("ip"), text -> Core.settings.put("ip", text)).size(320f, 54f).maxTextLength(100).addInputDialog().get();
	...
	void setup() {
		...
		cont.clear();
		cont.table(t -> {
			t.button(Icon.fileImage, () -> ui.showInfoText("Preview", "[coral][[[" + Random.getHex(player.color().rgba8888()) + "]" + player.name().trim() + "[coral]]:[white] Preview")).padRight(10);
		...
		hosts.table(name -> {
			...
			if (eye) {
				name.button(Icon.eyeSmall, Styles.emptyi, () -> {
					...
					refreshGlobal();
				}).update(i -> i.getStyle().imageUp = (showHidden ? Icon.eyeSmall : Icon.eyeOffSmall)).size(40f).right().padRight(3).tooltip("@servers.showhidden");
			}
